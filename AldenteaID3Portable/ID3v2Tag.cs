...
namespace Aldentea.ID3Portable
{
	...
	public abstract class ID3v2Tag : IID3Tag
	{
		...
		//protected Type StringFrameClass;
		//protected Type BinaryFrameClass;
		//protected Type EventTimeCodeFrameClass;
		...
		protected List<object> frames = new List<object>();
		...
		private static ID3v2Tag Generate(ID3Reader reader, bool only_header)
		{
			...
			switch (version[0])
			{
				...
				case 0x04:
					throw new Exception("残念ながらまだ未対応ですm(_ _)m");
				//tag = new ID3v23Tag();
				//break;
				default:
					throw new Exception("見たことないバージョンでチュね～");
			}
		}
		#endregion
