...
namespace Aldentea.ID3Portable
{
	...
	public class ID3v22Tag : ID3v2Tag
	{
		...
		protected override int AddFrame(string name, ID3Reader reader)
		{
			...
			if (name == "ETC")
			{
				frames.Add(new ID3v22EventTimeCodeFrame(name, reader));
				return frames.Count;
			}
			// テキストフレーム
			if (name[0] == 'T')
			{
				if (name != "TXX")
				{
					frames.Add(new ID3v22StringFrame(name, reader, true));
					return frames.Count;
				}
				else
				{
			...
			{
				frames.Add(new ID3v22BinaryFrame(name, reader));
				return frames.Count;
			}
		}
		#endregion
		...
		class ID3v22EventTimeCodeFrame : ID3v22Frame, IEventTimeCodeFrame
		{
			...
			List<TimeUnit?> event_time_units = new List<TimeUnit?>();
